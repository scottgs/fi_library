function [FMbinary] = fi_s_decomp( densities )
...
%	Outputs:
%		FMbinary = resultant S_decomposable (w.r.t. max operator) binary coded fuzzy measure
%
% Program description:
%   S-decomposable fuzzy measure given N densities

	nInputs = length(densities);
	vars = 1:2.^nInputs-1;
	varsBin = de2bi(vars, nInputs);
	FMbinary = arrayfun(@(x) max(densities(logical(varsBin(x,:)))), 1:length(vars)); 
	FMbinary(end) = 1;
